...
type MemoryItem struct {
	...
	Namespace       string        `gorm:"column:namespace;type:text;not null;index:idx_mem_namespace"`
	NamespaceType   NamespaceType `gorm:"column:namespace_type;type:text;not null;index:idx_mem_ns_type"`
	...
	Status          ItemStatus    `gorm:"column:status;type:text;default:'active';index:idx_mem_status"`
	ExpiresAt       *time.Time    `gorm:"column:expires_at;type:datetime;index:idx_mem_expires"`
	CreatedAt       time.Time     `gorm:"column:created_at;type:datetime;not null;index:idx_mem_created"`
	UpdatedAt       time.Time     `gorm:"column:updated_at;type:datetime;not null"`
	LastAccessAt    *time.Time    `gorm:"column:last_access_at;type:datetime"`
	...
	DedupeKey       *string       `gorm:"column:dedupe_key;type:text;uniqueIndex:idx_mem_dedupe"` // Unique within namespace
...
// Note: This is a virtual table and cannot be created via AutoMigrate.
// Use the migration SQL to create this table and its triggers.
